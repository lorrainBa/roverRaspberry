...
class EndToEndLaneFollower(object):
    ...
    def __init__(self,
                 car=None,
                 model_path='./models/lane_navigation.h5'):
    ...
    def compute_steering_angle(self, frame):
        self.center_ = """ Find the steering angle directly based on video frame
            We assume that camera is calibrated to point to dead center
        """
        preprocessed = img_preprocess(frame)
        X = np.asarray([preprocessed])
        logging.debug('compute steering angle using model')
        ...
        # steering angle fixed to 90 for calibration 
        #steering_angle = 90
